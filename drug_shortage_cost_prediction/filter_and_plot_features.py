"""
Feature Filtering and Visualization Script
Filters features by correlation using pre-computed analysis, then plots each feature against target variable
"""
...
def main():
    ...
    print("=" * 80)
    print("Feature Filtering and Visualization")
    print("=" * 80)
    
    # Load pre-computed information gain results
    print("\n1. Loading pre-computed Information Gain results...")
    ig_results = pd.read_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/information_gain_analysis.csv')
    print(f"   Loaded IG scores for {len(ig_results)} features")
    
    # Calculate median and filter features with IG >= median
    ig_cutoff = 0.17
    selected_features_ig = ig_results[ig_results['Information_Gain'] >= ig_cutoff].copy()
    selected_feature_names = selected_features_ig['Feature'].tolist()
    
    print(f"   Information Gain Cutoff: {ig_cutoff:.6f}")
    print(f"   Features with IG >= 0.17: {len(selected_feature_names)}")
    
    # Create IG dictionary for quick lookup
    ig_dict = {row['Feature']: row['Information_Gain'] for _, row in ig_results.iterrows()}
    
    # Load pre-computed correlation matrix results
    print("\n2. Loading pre-computed Correlation Matrix results...")
    corr_results = pd.read_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/correlation_matrix_analysis.csv')
    print(f"   Loaded correlation pairs for {len(corr_results)} feature pairs")
    
    # Filter correlation results to only include features with IG >= 0.17
    ...
    print(f"   Correlation pairs for features with IG >= 0.17: {len(corr_filtered)}")
    ...
    print("\n4. Loading original data for plotting...")
    ...
    print("\n6. Calculating correlation with target variable for each feature...")
    correlation_results = []
    ...
    for feat in final_features:
        ...
        correlation_results.append({
            ...
            'Abs_Correlation_with_Target': abs(corr_with_target),
            'Is_Categorical': feat in categorical_features
        })
    
    correlation_df = pd.DataFrame(correlation_results)
    correlation_df = correlation_df.sort_values('Abs_Correlation_with_Target', ascending=False)
    
    print(f"\n   Correlation with Target Statistics:")
    print(f"     Min: {correlation_df['Correlation_with_Target'].min():.6f}")
    print(f"     Median: {correlation_df['Correlation_with_Target'].median():.6f}")
    print(f"     Max: {correlation_df['Correlation_with_Target'].max():.6f}")
    print(f"     Mean: {correlation_df['Correlation_with_Target'].mean():.6f}")
    
    # Save correlation results
    correlation_df.to_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/feature_target_correlations.csv', index=False)
    print(f"\n   Saved: feature_target_correlations.csv")
    
    # Create plots directory
    plots_dir = '/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/feature_plots'
    ...
    for feat in final_features:
        ...
        corr_val = correlation_df[correlation_df['Feature'] == feat]['Correlation_with_Target'].values[0]
    ...
    print("=" * 80)
    print(f"\nFeatures with IG >= 0.17 ({ig_cutoff:.6f}): {len(selected_feature_names)}")
    ...
    print("=" * 80)
    print("\nOutput files:")
    print("  1. feature_target_correlations.csv - Correlation measures for each feature")
    print(f"  2. {plot_count} plot files in: feature_plots/")
