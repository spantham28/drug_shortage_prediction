...
Measures Information Gain and Correlation for all features
"""
...
def main():
    ...
    print("=" * 80)
    print("Feature Analysis - Information Gain and Correlation")
    print("=" * 80)
    ...
    exclude_cols = ['Provider CCN', target_col, "Profit"]
    ...
    print(f"\n   Target - Mean: {np.mean(y):,.2f}, Median: {np.median(y):,.2f}, "
          f"Std: {np.std(y):,.2f}, Skewness: {stats.skew(y):.4f}")
    ...
    print("\n4. Calculating Information Gain (Mutual Information)...")
    ...
    ig_results.to_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/information_gain_analysis.csv', index=False)
    print(f"\n   Saved: information_gain_analysis.csv")
    
    # Filter features for correlation analysis: only those with IG >= median
    ig_cutoff = 0.10
    selected_features_mask = mi_scores >= ig_cutoff
    ...
    print(f"     Features with IG >= 0.10: {len(selected_feature_names)}")
    ...
    print("\n5. Calculating Correlation Matrix (for features with IG >= 0.10)...")
    ...
    corr_results.to_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/correlation_matrix_analysis.csv', index=False)
    print(f"\n   Saved: correlation_matrix_analysis.csv")
    ...
    corr_matrix_df.to_csv('/Users/janakipantham/Desktop/drug_shortage_platform/drug_shortage_cost_prediction/correlation_matrix_square.csv')
    print(f"   Saved: correlation_matrix_square.csv (square matrix format)")
    ...
    print(f"  Features with IG >= 0.10 ({ig_cutoff:.6f}): {len(selected_feature_names)}")
    print(f"\nCorrelation (analyzed for {len(selected_feature_names)} features with IG >= 0.10):")
    ...
    print("=" * 80)
    print("\nOutput files:")
    print("  1. information_gain_analysis.csv - Information gain for each feature")
    print("  2. correlation_matrix_analysis.csv - Correlation pairs (long format)")
    print("  3. correlation_matrix_square.csv - Full correlation matrix (square format)")
